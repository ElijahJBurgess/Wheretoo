...
export function OrganizerDashboardPage() {
  ...
  if (query.isPending) {
    return <p role='status' className='operations-state'>Loading event dashboard…</p>
  }
  if (query.isError) {
    return (
      <OperationsError
        title='Event metrics unavailable'
  ...
  return (
    <section className='operations-page'>
      ...
      <EventHero metrics={data} />
      <div className='ops-actions'>
        {data.admissionEligible
          ? (
            <Link
        ...
        <Link className='ops-button' to={`/organizer/events/${eventId}/edit`}>Edit event</Link>
      </div>
      ...
      <p className='ops-note'>
        Remaining inventory includes active checkout reservations. Refunded inventory can be sold
        again.
      </p>
    </section>
  )
