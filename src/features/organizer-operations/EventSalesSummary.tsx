...
import { money } from './OperationsUi'
export function EventSalesSummary({ownerId,eventId}:{ownerId:string;eventId:string}) {
 ...
 return <span className="ops-event-sales"><span>{metrics.data.sold} / {metrics.data.capacity ?? '—'} sold</span><span>{money(metrics.data.grossSalesMinor)}</span></span>
}
