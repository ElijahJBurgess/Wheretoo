...
2. Capture the prior `private.checkout_runtime_control.checkout_creation_enabled` value in the release record. Set it to `false` in an approved maintenance window and verify it **before the authoritative zero-ticket check**. Closing new checkout creation does not expire existing Stripe Checkout Sessions or stop webhook delivery.
3. Reconcile in-flight work on both sides while the current database/function contract is still compatible: inspect nonterminal database orders, their exact Stripe Sessions/payment states, and pending or failed webhook receipts/deliveries. Expire still-open unpaid Sessions through the existing approved Stripe path and verify their final provider/database state; drain completed or processing payments and pending deliveries through verified webhook handling. A processing payment is not safely expired merely because its Session was closed. If this creates any durable ticket, stop for an owner decision; never delete or backfill tickets to pass the first-upgrade gate. Unresolved provider/database disagreement or an in-flight payment is a deployment blocker. If deliveries must wait across the cutover, first verify an approved durable retry path: retain the event/receipt and return a retryable failure while fulfillment is incompatible, then retry after the matching function/database versions are active. Do not acknowledge an unfulfilled event or assume disabling checkout pauses Stripe deliveries. If safe holding/retry cannot be demonstrated, stop the deployment.
4. With checkout still closed and in-flight sessions/deliveries accounted for, recheck `select count(*) from public.tickets;` immediately before the first Lite migration; it must be zero. The migration's own locked empty-ticket assertion is the final guard. Existing tickets require an owner decision. Subsequent releases preserve tickets and must not reuse this first-upgrade check as a deletion procedure. Preserve an existing `TICKET_CREDENTIAL_SECRET`. Only if absent and initial configuration is approved, generate **32 random bytes**, encode as canonical unpadded base64url (43 characters), and configure it once in the server secret store. Use a protected temporary file or secure deployment UI, with shell tracing off; keep an encrypted recoverable copy in the owner's secret store. It must have no `VITE_` name and must never enter source, fixtures, logs, browser configuration, or screenshots. The proof runner never sets, fetches, or unsets its value.
5. Keep the controlled window closed while coordinating the migrations, production functions, and app: apply committed migrations through `20260907010300`; deploy the updated `stripe-webhook`, `ticket-collection`, `ticket-admission`, and any changed checkout/confirmation functions; deploy the app built from the reviewed commit. The first migration replaces the fulfillment RPC contract, so the old webhook producer must not be treated as compatible during this interval. Confirm the matching functions are active, reconcile/retry any held deliveries, and verify provider/database agreement before the smoke proof. These are manual approved deployment steps, never runner side effects.
6. Run the local gates below, then approve the linked-development smoke proof. The proof temporarily enables checkout only after capturing the disabled prior state; on success, assertion failure, interrupt, or partial setup it settles the exact payment, cleans captured fixtures, and restores that prior state. Stop on any cleanup failure. Record safe command results and final commit, then reopen checkout to the prior appropriate state only after all launch gates pass and the owner approves opening sales.
...
Settlement allows at most 30 seconds per driver request and 120 seconds overall. A timeout aborts the request, marks settlement unsafe, and preserves financial rows; temporary-driver/temporary-secret retirement and failure reporting continue. A timed-out provider write has an unknown outcome and must be reconciled before retrying. After failure, keep checkout closed and retain the safe stage/command evidence. Do not force deletes, restore used tickets to valid, replace refunds, reset the credential secret, remove production functions, or erase audit history to make cleanup green. Resolve the exact captured fixture/provider state under an approved recovery procedure. An unrun or failed linked proof is an open launch gate.
