...
where bucket_digest = '$fixture_network' or bucket_digest in (
  '$fixture_actor', '$fixture_actor_network',
  $(for number in $(seq 1 31); do printf "'%064x'," "$number"; done | sed 's/,$//')
);
delete from private.event_reports where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from private.event_moderation_evaluations where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from private.event_public_eligibility_intervals where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from private.event_moderation_actions where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from private.event_policy_legacy_exemptions where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from private.event_risk_disclosures where event_id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
delete from public.events where id in ('$fixture_event'::uuid, '$fixture_actor_limit_event'::uuid);
...
"$supabase_cli" db query --linked "
  ...
  ) values ('actor', '$fixture_actor', clock_timestamp(), 9, clock_timestamp() + interval '1 hour');
  commit;
...
# Two events let concurrent sessions contend solely on the same actor bucket.
# A pre-existing count of nine permits exactly one of these calls to reach ten.
submit "$fixture_event" "$fixture_actor" "$fixture_actor_network" & actor_first_pid=$!
submit "$fixture_actor_limit_event" "$fixture_actor" "$fixture_actor_network" & actor_second_pid=$!
wait "$actor_first_pid"
wait "$actor_second_pid"

# Two simultaneous submissions from the same actor must serialize to one report.
same_actor="$(printf '%064x' 1)"
submit "$fixture_event" "$same_actor" "$fixture_network" & first_pid=$!
submit "$fixture_event" "$same_actor" "$fixture_network" & second_pid=$!
wait "$first_pid"
wait "$second_pid"

# Fill one network bucket with 29 more distinct actors; the 31st distinct actor
# must be atomically denied, while the first three create exactly one job.
for number in $(seq 2 31); do
  submit "$fixture_event" "$(printf '%064x' "$number")" "$fixture_network" &
done
wait
...
"$supabase_cli" db query --linked "
  do \$assert\$
  begin
    if (select count(*) from private.event_reports where event_id = '$fixture_event'::uuid and status = 'open') <> 31 then
    ...
    if (select count(*) from private.event_moderation_evaluations where event_id = '$fixture_event'::uuid
      and source = 'report' and status = 'queued') <> 1 then
      raise exception using errcode = 'P0001', message = 'ASSERT_ONE_REPORT_EVALUATION';
    end if;
    if not exists (select 1 from public.events where id = '$fixture_event'::uuid
...
echo 'Report concurrency dedupe, atomic limit, and single escalation passed.'
