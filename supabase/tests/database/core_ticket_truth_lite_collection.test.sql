...
select no_plan();
select is(to_regprocedure('public.server_lookup_lite_ticket_collection(text)'),null::regprocedure,'obsolete collection RPC does not remain executable');
select has_function('public','server_lookup_paid_ticket_collection',array['text'],'service collection projection exists');
select ok(not has_function_privilege('anon','public.server_lookup_paid_ticket_collection(text)','EXECUTE'),'anon cannot read collections');
select ok(not has_function_privilege('authenticated','public.server_lookup_paid_ticket_collection(text)','EXECUTE'),'authenticated cannot read collections');
select ok(has_function_privilege('service_role','public.server_lookup_paid_ticket_collection(text)','EXECUTE'),'service role can read collections');
set local role anon;
select throws_ok($$select * from public.server_lookup_paid_ticket_collection(repeat('1',64))$$,'42501',null,'anonymous RPC execution denied');
reset role;
set local role authenticated;
select throws_ok($$select * from public.server_lookup_paid_ticket_collection(repeat('1',64))$$,'42501',null,'authenticated RPC execution denied');
reset role;
...
select is((select quantity from public.server_lookup_paid_ticket_collection(p_confirmation_token_hash => repeat('1',64))),3,'paid collection has exact total');
select is((select jsonb_array_length(tickets) from public.server_lookup_paid_ticket_collection(repeat('1',64))),3,'every purchased ticket returned');
select is((select count(*) from public.server_lookup_paid_ticket_collection(repeat('2',64))),0::bigint,'unpaid collection unavailable');
select is((select count(*) from public.server_lookup_paid_ticket_collection(repeat('f',64))),0::bigint,'unknown bearer unavailable');
select is((select count(*) from public.server_lookup_paid_ticket_collection('malformed')),0::bigint,'malformed hash unavailable');
select is((select count(*) from public.server_lookup_paid_ticket_collection(null)),0::bigint,'null hash unavailable');
reset role;
...
begin
  ...
  select count(*) into v_count from public.server_lookup_paid_ticket_collection(repeat('1',64));
...
select is((select count(*) from public.server_lookup_paid_ticket_collection(repeat('1',64))),1::bigint,'used history alongside valid tickets remains available');
reset role;
...
set local role service_role;
select is((select count(*) from public.server_lookup_paid_ticket_collection(repeat('1',64))),1::bigint,'paid order on cancelled event retains coherent used and cancelled collection');
reset role;
...
set local role service_role;
select is((select count(*) from public.server_lookup_paid_ticket_collection(repeat('1',64))),1::bigint,'fully refunded collection retains used history');
select * from finish();
rollback;
