...
select public.server_finish_ticket_email_dispatch((v->>'id')::uuid,(v->>'lease_id')::uuid,'failed') from claims;
select extensions.ok((select bool_and(t.status=case when h.status='used' then 'used' else 'refunded' end and t.used_at is not distinct from h.used_at) from public.tickets t join history h using(id)),'failed notice delivery never alters financial/admission truth or used timestamp');

select extensions.is((select status from public.orders where id=(select id from fulfillment_orders where kind='clean')),'refunded','failed refund_notice never changes financial completion');
select extensions.is((select confirmation_status from public.server_lookup_checkout_integrity_confirmation(repeat('1',64))),'refunded','failed refund_notice never changes original buyer payment confirmation');
select extensions.is(public.get_organizer_refund_status('a6200000-0000-4000-8000-000000000001',(select id from fulfillment_orders where kind='clean'))->>'state','completed','organizer financial state remains complete despite notice failure');
select extensions.is(public.get_organizer_refund_notice_status('a6200000-0000-4000-8000-000000000001',(select id from fulfillment_orders where kind='clean'))->>'state','failed','delivery state reports failure independently');
