-- Complete the bounded staff-console producer contract. These projections
-- expose only aggregate review/report signals and the owner ID required for
-- exact cache invalidation; private report/reviewer facts remain unavailable.

drop function public.list_moderation_queue(integer);
...
begin
  ...
  select
    ...
    coalesce(evaluation_signal.evaluation_count, 0::bigint),
    evaluation_signal.oldest_queued_at
  ...
  left join lateral (
    select
      count(evaluations.id)::bigint as evaluation_count,
      min(evaluations.created_at) as oldest_queued_at
  ...
  order by
    ...
    coalesce(review_signal.is_open, false) desc,
    coalesce(report_signal.report_count, 0::bigint) desc,
    evaluation_signal.oldest_queued_at nulls last,
...
from public, anon, authenticated, service_role;
grant execute on function public.list_moderation_queue(integer)
to authenticated;
