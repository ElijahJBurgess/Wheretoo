...
declare
  ...
  v_can_clear_organizer_edit_hold boolean := false;
  v_paid_tiers_are_active boolean := false;
  v_candidate boolean;
begin
  ...
  if v_event.starts_at is null
    ...
    or v_event.ends_at <= pg_catalog.now()
    or v_event.ends_at <= v_event.starts_at
    or (
      v_event.status = 'draft'
      and v_event.starts_at <= pg_catalog.now()
    ) then
    raise exception using
      errcode = 'P0001',
  ...
  select
    exists (
      ...
      where actions.event_id = v_event.id
        ...
        and actions.actor_user_id = v_event.organizer_id
    ...
    and not exists (
      ...
      where actions.event_id = v_event.id
        ...
        and actions.moderation_version = v_event.moderation_version
        and (
          actions.actor_type <> 'organizer'
          or actions.actor_user_id is distinct from v_event.organizer_id
          or actions.source <> 'edit'
        )
    )
  into v_can_clear_organizer_edit_hold;
  ...
  if v_event.status = 'published'
    and v_event.admission_type = 'paid' then
    select
      pg_catalog.count(*) between 1 and 3
      and coalesce(pg_catalog.bool_and(ticket_tiers.status = 'active'), false)
    into v_paid_tiers_are_active
    from public.ticket_tiers
    where ticket_tiers.event_id = p_event_id
      and ticket_tiers.status <> 'archived';
  end if;
  ...
  if v_event.status = 'draft' then
    ...
    and v_event.moderation_status not in ('blocked', 'removed')
    and not v_paid_tiers_are_active then
    select activated.*
    into v_event
  ...
  if v_event.moderation_status in ('blocked', 'removed') then
    null;
  elsif v_event.moderation_status = 'under_review'
    and not v_can_clear_organizer_edit_hold then
    null;
