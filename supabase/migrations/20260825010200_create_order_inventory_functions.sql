...
begin
  if p_order_id is null
    ...
    or p_session_id <> lower(p_session_id)
    or p_session_id !~ '^cs_test_[a-z0-9]+$'
...
create or replace function private.cancel_checkout_reservation(
  p_order_id uuid,
  p_reason text
)
returns uuid
...
begin
  if p_order_id is null
    or v_reason is null
    or char_length(v_reason) not between 1 and 255 then
    raise exception using errcode = 'P0001', message = 'CHECKOUT_INPUT_INVALID';
  end if;
  ...
  if v_order.status in ('creating_checkout', 'checkout_open') then
    update public.orders as orders
    set status = 'cancelled',
        failed_at = coalesce(orders.failed_at, statement_timestamp()),
        failure_code = coalesce(orders.failure_code, v_reason)
    where orders.id = p_order_id;
  end if;
...
grant execute on function private.reserve_checkout(uuid, uuid, text, text, uuid, text)
to service_role;
grant execute on function private.attach_checkout_session(uuid, text, timestamptz)
to service_role;
grant execute on function private.cancel_checkout_reservation(uuid, text)
to service_role;
grant execute on function private.expire_checkout_reservations(timestamptz)
to service_role;
