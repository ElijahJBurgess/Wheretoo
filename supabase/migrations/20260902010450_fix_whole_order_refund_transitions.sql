alter function private.prepare_whole_order_refund(uuid, text)
  rename to prepare_whole_order_refund_v10400;
...
begin
  ...
  if not exists (
    ...
    where orders.id = p_order_id and orders.status = 'paid'
  ) or exists (
  ...
  return query
  select * from private.prepare_whole_order_refund_v10400(
    p_order_id, p_reason
  );
end;
$$;
...
) rename to apply_verified_refund_v10400;
...
begin
  ...
  if found
    and v_refund.status in ('pending', 'requires_action')
  ...
  return query
  select * from private.apply_verified_refund_v10400(
...
revoke all on function private.prepare_whole_order_refund_v10400(uuid, text)
  from public, anon, authenticated, service_role;
revoke all on function private.prepare_whole_order_refund(uuid, text)
  from public, anon, authenticated, service_role;
revoke all on function private.apply_verified_refund_v10400(
  text, uuid, text, text, text, text, text, bigint, text, text, text,
  boolean, boolean, bigint, bigint, boolean, text
) from public, anon, authenticated, service_role;
...
comment on function private.prepare_whole_order_refund_v10400(uuid, text) is
  'Task 8 implementation detail; call the guarded public service boundary.';
comment on function private.apply_verified_refund_v10400(
  text, uuid, text, text, text, text, text, bigint, text, text, text,
  boolean, boolean, bigint, bigint, boolean, text
) is 'Task 8 implementation detail; exact pending evidence advances through the guarded wrapper.';
