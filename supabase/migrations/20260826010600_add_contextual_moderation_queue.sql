create function public.server_claim_moderation_evaluation(p_worker_reference text)
returns table (
  evaluation_id uuid,
  event_id uuid,
...
begin
  ...
  where evaluations.status = 'queued'
...
create function public.server_apply_moderation_evaluation(
...
begin
  if p_evaluation_id is null
    ...
    or (p_provider_reference is not null and (
      pg_catalog.char_length(p_provider_reference) < 1
      or pg_catalog.char_length(p_provider_reference) > 255
    ))
    or (p_model_version is not null and (
      pg_catalog.char_length(p_model_version) < 1
      or pg_catalog.char_length(p_model_version) > 120
    )) then
    raise exception using
      errcode = '22023',
  ...
  if v_evaluation.status = 'succeeded' then
    ...
    return 'superseded';
  elsif v_evaluation.status <> 'processing'
    or v_evaluation.source not in ('contextual', 'report')
    or v_evaluation.content_revision <> p_content_revision
    or v_evaluation.input_sha256 <> p_input_sha256
    or v_evaluation.queued_moderation_version <> p_queued_moderation_version then
    raise exception using
      errcode = 'P0001',
      message = 'MODERATION_EVALUATION_CONFLICT';
  end if;
  ...
  if v_event.content_revision <> p_content_revision
    ...
    set status = 'superseded',
        finished_at = v_now,
        failure_code = 'STALE_EVALUATION'
    where evaluations.id = p_evaluation_id;
    return 'superseded';
  end if;

  v_new_status := case
    when p_outcome = 'clear_candidate' then 'clear'
    else 'under_review'
  end;
  v_reason_code := case
    when p_outcome = 'clear_candidate' then 'no_violation'
    else p_reason_codes[1]
  end;
  ...
  if v_event.moderation_status = v_new_status
    and v_new_status = 'clear'
  ...
  set moderation_status = v_new_status,
      moderated_revision = case when v_new_status = 'clear'
        then events.content_revision else null end,
      moderation_version = v_new_version,
      moderation_updated_at = v_now
  where events.id = v_event.id
    and events.content_revision = p_content_revision
  ...
  where evaluations.event_id = v_event.id
    and evaluations.id <> p_evaluation_id
  ...
  insert into private.event_moderation_actions (
    ...
    v_event.id, v_event.content_revision, p_input_sha256, 'system',
    'evaluation', case when v_new_status = 'clear' then 'clear' else 'hold' end,
    v_event.moderation_status, v_new_status, v_event.public_history_status,
    v_event.public_history_status, v_reason_code, p_evaluation_id,
    v_new_version, v_now
...
create function public.server_fail_moderation_evaluation(
  p_evaluation_id uuid,
  p_content_revision bigint,
  p_input_sha256 text,
  p_queued_moderation_version bigint,
...
begin
  if p_evaluation_id is null
    ...
    raise exception using
      errcode = '22023',
      message = 'MODERATION_FAILURE_INVALID';
  end if;
...
revoke all on function public.server_claim_moderation_evaluation(text)
from public, anon, authenticated, service_role;
revoke all on function public.server_apply_moderation_evaluation(
  uuid, bigint, text, bigint, text, text, text[], text, text
) from public, anon, authenticated, service_role;
revoke all on function public.server_fail_moderation_evaluation(
  uuid, bigint, text, bigint, text
) from public, anon, authenticated, service_role;

grant execute on function public.server_claim_moderation_evaluation(text)
to service_role;
grant execute on function public.server_apply_moderation_evaluation(
  uuid, bigint, text, bigint, text, text, text[], text, text
) to service_role;
grant execute on function public.server_fail_moderation_evaluation(
  uuid, bigint, text, bigint, text
) to service_role;
