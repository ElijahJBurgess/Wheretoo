alter table public.refunds
  ...
  add constraint refunds_payment_intent_id_check check (
  ...
  add constraint refunds_charge_id_check check (
  ...
  add constraint refunds_status_check check (
    status in ('pending', 'requires_action', 'succeeded', 'failed', 'canceled', 'cancelled')
  );

drop function public.server_apply_refund(
  text, uuid, text, bigint, text, text, text, boolean, boolean
);
drop function private.apply_refund(
  text, uuid, text, bigint, text, text, text, boolean, boolean
);

create function private.apply_refund(
...
begin
  ...
  if not found
    ...
    or p_payment_intent_id is distinct from v_order.stripe_payment_intent_id
    or p_charge_id is null
    or p_charge_id is distinct from v_order.stripe_charge_id
  ...
  values (
    ...
    p_amount_minor, p_currency, p_status, p_reason, p_reverse_transfer,
...
create function public.server_apply_refund(
  p_stripe_event_id text,
  p_order_id uuid,
  p_stripe_refund_id text,
  p_payment_intent_id text,
...
as $$
  select * from private.apply_refund(
    ...
    p_payment_intent_id, p_charge_id, p_amount_minor, p_currency,
    p_status, p_reason, p_reverse_transfer, p_refund_application_fee
  );
$$;

revoke all on function private.apply_refund(
  text, uuid, text, text, text, bigint, text, text, text, boolean, boolean
