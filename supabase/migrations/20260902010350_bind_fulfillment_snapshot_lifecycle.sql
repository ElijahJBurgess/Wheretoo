-- Forward-only correction for the checkout-integrity fulfillment boundary.
-- The applied 20260902010300 migration remains immutable.
...
begin
  ...
  v_legacy_tier_id := (v_digest_items -> 0 ->> 'ticket_tier_id')::uuid;

  -- Cart orders bind the sorted complete item snapshots. The retained singular
  -- reservation overload uses the legacy digest, so accept that exact form only
  -- for a one-item order. Both stored digests are write-once after reservation.
  v_item_snapshot_valid :=
    ...
    or (
      v_item_count = 1
      and v_order.stripe_checkout_request_digest = private.checkout_request_digest(
        v_order.id,
        v_order.event_id,
        v_legacy_tier_id,
        v_order.client_request_id,
