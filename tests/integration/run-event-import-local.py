#!/usr/bin/env python3
"""Manage only disposable Event Import proof stacks. Never reads a linked project."""
...
if args.command == 'start':
    ...
    result = subprocess.run(cli + ['db', 'reset', '--local', '--workdir', str(local), '--yes'], cwd=ROOT, stdout=subprocess.DEVNULL)
    if result.returncode:
        subprocess.run(['docker','exec','supabase_db_'+project,'psql','-X','-U','postgres','-At','-c',"select cron.alter_job(jobid,active:=false) from cron.job;"],check=False,stdout=subprocess.DEVNULL)
        raise SystemExit('Local reset returned a container-health error; cron disabled where available. Inspect the log and migration ledger before testing.')
