"""Dedicated loopback storefront proof stack. Never reads a linked project."""
import json, pathlib, re, subprocess, sys, shutil
ROOT=pathlib.Path(__file__).resolve().parents[2]
LOCAL=ROOT/'.supabase/storefront'
PROJECT='wheretoo-storefront'
CLI=['pnpm','exec','supabase']
def prepare():
 target=LOCAL/'supabase'; target.mkdir(parents=True,exist_ok=True)
 config=(ROOT/'supabase/config.toml').read_text().replace('543','573')
...
if __name__=='__main__':
 ...
 if action=='start':
  subprocess.run(CLI+['start','--workdir',str(LOCAL),'--exclude','studio,imgproxy,logflare,vector,supavisor,inbucket,realtime'],cwd=ROOT,check=True)
  sql('select cron.alter_job(jobid,active:=false) from cron.job;')
  print('Dedicated storefront local stack: 127.0.0.1:57321')
 elif action=='reset':
