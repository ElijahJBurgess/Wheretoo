#!/usr/bin/env python3
"""Start/verify only a labelled Spec 10 local REST facade; never a remote service."""
import importlib.util, json, pathlib, subprocess, sys
ROOT=pathlib.Path(__file__).resolve().parents[2]
spec=importlib.util.spec_from_file_location('spec10db',ROOT/'tests/integration/spec10-database.py')
db=importlib.util.module_from_spec(spec);spec.loader.exec_module(db)
NAME='whereto-spec10-spec11-rest'
IDENTITY=ROOT/'.superpowers/spec10-spec11/rest-identity.json'
...
if __name__=='__main__':
    if len(sys.argv)>1 and sys.argv[1]=='restart':
        ...
        assert live['Id']==known['id'] and live['Name']=='/'+NAME
        assert live['Config']['Labels'].get('wheretoo.task')=='spec10-spec11-integration'
        assert live['HostConfig']['PortBindings']['3000/tcp']==[{'HostIp':'127.0.0.1','HostPort':'55546'}]
        db.sql("alter role authenticator with login password 'spec10-spec11-disposable-only';")
        subprocess.run(['docker','start',known['id']],check=True,capture_output=True)
    if len(sys.argv)>1 and sys.argv[1]=='create':
        ...
        result=subprocess.check_output(['docker','run','--pull','never','--name',NAME,'--label','wheretoo.task=spec10-spec11-integration','-p','127.0.0.1:55546:3000',
          '-e','PGRST_DB_URI=postgres://authenticator:spec10-spec11-disposable-only@host.docker.internal:55545/postgres',
          '-e','PGRST_DB_SCHEMAS=public','-e','PGRST_DB_ANON_ROLE=anon',
          '-e','PGRST_JWT_SECRET=spec10-local-only-jwt-secret-disposable-2026',
    ...
    print('Verified Spec 10 local REST:',verify())
