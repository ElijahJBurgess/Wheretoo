import { RefundEvidenceConflict, refundModeIsTestCompatible, recoverExistingRefundEvidence as recoverRefundEvidence, type ExistingRefundSnapshot as RefundEvidenceSnapshot } from "../../../../supabase/functions/_shared/refundEvidenceRecovery.ts";
...
// The original fixture wrapper keeps its fixed economic contract; runtime callers
// supply durable amounts to the same evidence validator.
export type ExistingRefundSnapshot = Omit<RefundEvidenceSnapshot, "totalMinor" | "applicationFeeAmountMinor">;
export { refundModeIsTestCompatible };
export function recoverExistingRefundEvidence(snapshot: ExistingRefundSnapshot, dependencies: Parameters<typeof recoverRefundEvidence>[1]) {
 return recoverRefundEvidence({...snapshot,totalMinor:5500,applicationFeeAmountMinor:425},dependencies);
}
