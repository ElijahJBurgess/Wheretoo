...
const jwtSecret = "spec09-local-only-jwt-secret-disposable-2026";
const key = new Uint8Array(32).fill(7);
const workerSecret = "spec07-local-worker-invocation-only-2026";
const webhookSecret = "whsec_" + btoa("spec07-local-webhook-signature-key");
...
async function verifyDisposable() {
  const ids = await Promise.all(
    ["database", "rest"].map(async (name) =>
      JSON.parse(
        await Deno.readTextFile(`.superpowers/spec08-spec09/${name}-identity.json`),
      ).id as string
    ),
  );
...
const recovery = createTicketEmailHttpHandler("recovery", httpDependencies);
...
const collection = createTicketCollectionHandler({
  appOrigin: origin,
  getCredentialSecret: () => key,
  findCollection: async (hash) =>
    (await rpc("server_lookup_paid_ticket_collection", {
      p_confirmation_token_hash: hash,
    }))[0] ?? null,
  findFreeCollection: (hash) =>
    rpc("server_lookup_free_ticket_collection", { p_access_hash: hash }),
});
const free = createFreeRsvpHandler("create", {
  appOrigin: origin,
  getSecret: () => key,
  rpc,
...
async function observation(message: Captured, kind = "delivered") {
  ...
  return webhook(
    new Request("http://127.0.0.1:55557/functions/v1/ticket-email-webhook", {
      method: "POST",
      headers: {
        "svix-id": id,
...
const config: Record<string, string> = {
  ...
  RESEND_API_KEY: "re_spec07_never_real",
  APP_BASE_URL: origin,
...
await verifyDisposable();
Deno.serve({ hostname: "127.0.0.1", port: 55557 }, async (request) => {
  const url = new URL(request.url), path = url.pathname;
  if (path === "/health") return new Response("spec07-disposable");
  if (path === "/functions/v1/ticket-recovery-request") {
  ...
  if (path === "/__spec07/messages") {
    return Response.json([...messages.values()]);
  }
  if (path === "/__spec07/work" && request.method === "POST") {
    ...
    return worker(
      new Request("http://127.0.0.1:55557/functions/v1/ticket-email-worker", {
        method: "POST",
        headers: { authorization: "Bearer " + workerSecret },
      }),
    );
  }
  if (path === "/__spec07/observe" && request.method === "POST") {
    const input = await request.json(),
  ...
  return new Response("Outside disposable Spec 07 proof", { status: 404 });
});
