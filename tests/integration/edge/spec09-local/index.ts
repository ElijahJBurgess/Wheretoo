...
import { localRefund, refundProof } from "./refund.ts";
...
const workerSecret = "spec09-local-worker-invocation-only-2026";
const webhookSecret = "whsec_" + btoa("spec09-local-webhook-signature-key");
...
async function observation(message: Captured, kind = "delivered") {
  ...
  return webhook(
    new Request("http://127.0.0.1:55567/functions/v1/ticket-email-webhook", {
      method: "POST",
      headers: {
        "svix-id": id,
...
const config: Record<string, string> = {
  ...
  RESEND_API_KEY: "re_spec09_never_real",
  APP_BASE_URL: origin,
...
const confirmationClient = createClient("http://127.0.0.1:55567", token("service_role"), {
...
const organizerRefund = localRefund(rpc);
await verifyDisposable();
Deno.serve({ hostname: "127.0.0.1", port: 55567 }, async (request) => {
  ...
  if (path === "/functions/v1/organizer-refund-order") {
    return organizerRefund(request);
  }
  if (path === "/__spec09/refund-proof") return Response.json(refundProof);
  if (path === "/health") return new Response("spec09-disposable");
  ...
  if (path === "/__spec09/messages") {
    return Response.json([...messages.values()]);
  }
  if (path === "/__spec09/work" && request.method === "POST") {
    ...
    return worker(
      new Request("http://127.0.0.1:55567/functions/v1/ticket-email-worker", {
        method: "POST",
        headers: { authorization: "Bearer " + workerSecret },
      }),
    );
  }
  if (path === "/__spec09/observe" && request.method === "POST") {
    const input = await request.json(),
