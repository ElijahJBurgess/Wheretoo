...
spec=importlib.util.spec_from_file_location('local',ROOT/'tests/integration/storefront-local.py'); local=importlib.util.module_from_spec(spec);spec.loader.exec_module(local)
CONFIG=json.loads(subprocess.check_output(['pnpm','exec','supabase','status','--workdir',str(local.LOCAL),'-o','json'],cwd=ROOT,text=True,stderr=subprocess.DEVNULL));BASE=CONFIG['API_URL'];assert BASE=='http://127.0.0.1:57321'
def http(path,token=None,body=None,method='POST',mime='application/json',origin='http://127.0.0.1:3070'):
