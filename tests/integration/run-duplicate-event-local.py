"""Manage only the dedicated local Duplicate-event proof stack; never reads a linked project."""
import base64
import struct
import zlib
import json
...
PROJECT = 'wheretoo-duplicate-event'
...
def prepare():
    ...
    config = config[:config.index('[functions.')] + ''.join(f'\n[functions.{name}]\nverify_jwt = false\nimport_map = "../deno.json"\n' for name in ['event-images','event-cover-generation','duplicate-event'])
    ...
    if (ROOT / 'supabase/functions/duplicate-event').exists():
        files['duplicate-event'] = ['index.ts', 'duplicateEvent.ts']
...
if command not in ('start', 'reset', 'serve', 'test', 'stop'):
    raise SystemExit('Usage: python3 tests/integration/run-duplicate-event-local.py start|reset|serve|test|stop')
prepare()
...
if command == 'start':
    ...
    print('Local Duplicate-event stack ready on 127.0.0.1:58321. No keys printed.')
elif command == 'reset':
    ...
    subprocess.run(['docker', 'exec', 'supabase_db_' + PROJECT, 'psql', '-X', '-U', 'postgres', '-At', '-v', 'ON_ERROR_STOP=1', '-c', "select private.configure_policy_environment('development'); select cron.alter_job(jobid,active:=false) from cron.job;"], check=True, stdout=subprocess.DEVNULL)
    subprocess.run(['docker', 'exec', 'supabase_db_' + PROJECT, 'mkdir', '-p', '/tmp/duplicate-proof-tests'], check=True)
    subprocess.run(['docker', 'cp', str(ROOT / 'supabase/tests') + '/.', 'supabase_db_' + PROJECT + ':/tmp/duplicate-proof-tests'], check=True)
    for name in ['duplicate_event.test.sql', 'duplicate_event_history.test.sql', 'ai_event_cover.test.sql', 'spec15_event_images.test.sql','ai_cover_generation.test.sql']:
        result = subprocess.run(['docker', 'exec', 'supabase_db_' + PROJECT, 'psql', '-X', '-U', 'postgres', '-At', '-v', 'ON_ERROR_STOP=1', '-f', '/tmp/duplicate-proof-tests/database/' + name], text=True, capture_output=True, check=True)
        if 'not ok ' in result.stdout:
            raise SystemExit(result.stdout)
        print(name + ': ' + next(line for line in result.stdout.splitlines() if line.startswith('1..')))
    subprocess.run([sys.executable, str(ROOT / 'tests/integration/duplicate-event-proof.py')], cwd=ROOT, check=True)
